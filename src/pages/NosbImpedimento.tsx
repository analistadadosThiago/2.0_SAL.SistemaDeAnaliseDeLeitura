...
import { NosbImpedimentoData } from '../types';
import jsPDF from 'jspdf';
import autoTable from 'jspdf-autotable';
import * as XLSX from 'xlsx';

export default function NosbImpedimento() {
  ...
  // Data State
  const [results, setResults] = useState<NosbImpedimentoData[]>([]);
  ...
  const fetchFilterOptions = useCallback(async () => {
    ...
    try {
      const { data: filtersData, error: filtersError } = await supabase.rpc('get_filtros_nosb');

      if (filtersError) {
        // Fallback to individual calls if get_filtros_nosb doesn't exist
        const { data: anosData } = await supabase.rpc('dashboard_getanos');
        const { data: mesesData } = await supabase.rpc('dashboard_getmeses');
        const { data: razoesData } = await supabase.rpc('dashboard_getrazoes');
        const { data: matrData } = await supabase.rpc('get_lista_matriculas');
        const { data: motivosData } = await supabase.rpc('get_lista_motivos_nosb');

        const anos = (anosData || []).map((item: any) => String(typeof item === 'object' ? item.ano : item)).sort((a: string, b: string) => Number(b) - Number(a));
        const meses = (mesesData || []).map((item: any) => String(typeof item === 'object' ? item.mes : item));
        const razoes = (razoesData || []).map((item: any) => String(typeof item === 'object' ? item.rz : item)).sort((a: string, b: string) => Number(a) - Number(b));
        const matriculas = (matrData || []).map((item: any) => String(typeof item === 'object' ? item.matr : item)).sort();
        const motivos = (motivosData || [])
          .map((item: any) => String(typeof item === 'object' ? item.motivo : item))
  ...
  const handleGenerate = async () => {
    ...
    try {
      const { data, error: rpcError } = await supabase.rpc('get_nosb_impedimento', {
        p_ano: ano || "",
        p_mes: mes || "",
        p_rz: razao || "",
        p_matr: matr || "",
      ...
      // Filter out "Faturada"
      const filteredData = (data || []).filter((r: any) => r.f_motivo !== 'Faturada');
  ...
  const groupedByRazao = useMemo(() => {
    ...
    results.forEach(r => {
      const rz = String(r.f_rz || "");
      const mot = String(r.f_motivo || "");
      const key = `${rz}_${mot}`;
      if (!grouped[key]) {
  ...
  const groupedByMatricula = useMemo(() => {
    const grouped: { [key: string]: { matr: string, razao: string, motivo: string, quantidade: number } } = {};
    results.forEach(r => {
      const m = String(r.f_matr || "");
      const rz = String(r.f_rz || "");
      const mot = String(r.f_motivo || "");
      const key = `${m}_${rz}_${mot}`;
      if (!grouped[key]) {
        grouped[key] = { matr: m, razao: rz, motivo: mot, quantidade: 0 };
      }
      grouped[key].quantidade += 1;
    });
  ...
  const exportToPDF = () => {
    ...
    if (activeTab === 'ul') {
      tableColumn = ["MÊS", "ANO", "RAZÃO", "UL", "INSTALAÇÃO", "MEDIDOR", "REG", "TIPO", "MATR", "COD", "LEITURA", "MOTIVO"];
      tableRows = results.map(r => [
        r.f_mes, r.f_ano, r.f_rz, r.f_ul, r.f_instalacao, r.f_medidor, r.f_reg, r.f_tipo, r.f_matr, r.f_nl, r.f_l_atual, r.f_motivo
      ]);
    } else if (activeTab === 'razao') {
      tableColumn = ["RAZÃO", "MOTIVO", "QUANTIDADE"];
      tableRows = groupedByRazao.map(r => [r.razao, r.motivo, r.quantidade]);
    } else {
      tableColumn = ["MATRÍCULA", "RAZÃO", "MOTIVO", "QUANTIDADE"];
      tableRows = groupedByMatricula.map(r => [r.matr, r.razao, r.motivo, r.quantidade]);
    }

    doc.setFontSize(18);
    doc.text(`SAL - N’OSB - Impedimento (${activeTab.toUpperCase()})`, 14, 15);
    doc.setFontSize(10);
    ...
    doc.save(`SAL_Nosb_Impedimento_${activeTab}_${new Date().getTime()}.pdf`);
  };
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(workbook, `SAL_Nosb_Impedimento_${activeTab}_${new Date().getTime()}.xlsx`);
  };
  ...
  return (
    <div className="space-y-8 pb-12">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-bold text-zinc-900 tracking-tight">N’OSB - Impedimento</h1>
          <p className="text-zinc-500 text-sm mt-1">Análise de impedimentos de leitura e não impressão de faturas</p>
        </div>
                    ...
                    <tr key={i} className="hover:bg-zinc-50 transition-colors">
                      {activeTab === 'ul' && (
                        <>
                          ...
                          <td className="px-4 py-3 text-xs text-zinc-600 font-medium">{r.f_rz}</td>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
              <div className="flex items-center gap-6">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-lg font-bold text-zinc-900">Grafico de dados Nosb Impressao</h3>
                </div>
      ...
      {!loading && !hasGenerated && !error && (
        <div className="h-[50vh] flex flex-col items-center justify-center text-center space-y-4 bg-white rounded-[32px] border border-zinc-100 shadow-sm">
          ...
          <p className="text-zinc-500 max-w-xs">Preencha os filtros acima e clique em Gerar para visualizar os dados de impedimento.</p>
        </div>
      )}
    </div>
  );
